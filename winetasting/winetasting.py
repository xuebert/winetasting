...
class WineTasting:
    ...
    def generate_tasting_format(self, randomize_order: bool = True, seed=None) -> None:

        assert len({'wine_id', 'country', 'brand', 'wine_type', 'price'} - set(self.answer_df.columns)) == 0

        self.tasting_df = self.answer_df[['wine_id']]
        if randomize_order:
            self.tasting_df = self.tasting_df.sample(self.tasting_df.shape[0], random_state=seed)
    ...
    def get_results(self):
        ...
        type_accuracy_df = type_accuracy_df.merge(analysis_df['wine_type'].value_counts().reset_index().rename(columns={'index': 'wine_type', 'count': 'total_guesses'}))
        ...
        bottle_accuracy_df = bottle_accuracy_df.merge(analysis_df['wine_id'].value_counts().reset_index().rename(columns={'index': 'wine_id', 'count': 'total_guesses'}))
        ...
        analysis_df['abs_percent_price_diff'] = (100 * analysis_df['abs_price_diff'] / analysis_df['price']).round()
