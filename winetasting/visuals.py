...
def create_accuracy_visuals(analysis_df):
    ...
    labels = {'correct_type': 'Correct', 'wine_type': 'Wine type', 'count': 'Count',
              'price': 'Price ($)', 'wine_type__brand': 'Wine type & brand', 'relative_price_diff': 'Mean abs difference (%)'}
    ...
    plot_df['wine_type__brand'] = plot_df['wine_type'] + '__' + plot_df['brand']
    plot_df = plot_df.groupby(['wine_type__brand', 'price', 'tasting_order'])['abs_price_diff'].mean().reset_index()
    plot_df['relative_price_diff'] = 100*plot_df['abs_price_diff'] / plot_df['price']
    plot_df = plot_df.sort_values(['relative_price_diff'], ascending=False)
    fig = px.bar(plot_df, x='wine_type__brand', y='relative_price_diff', color='relative_price_diff', labels=labels, color_continuous_scale=px.colors.sequential.Bluered_r)
    fig.show()
